...
- 增加“新增宠物任务”按钮(DONE)
- 增加“强化任务”按钮(DONE)
- 增加展示字段“奖励星星”,"奖励徽章"，"奖励奖杯"
##### 7.2.1 宠物任务(TODO)
##### 7.2.2 强化任务(TODO)

#### 7.3 商场管理
- 修改商场管理的数据模型（字段：商品名称、商品照片、商品类别、商品价值、商品价值、商品描述、商品状态），移除效果值和效果描述（DONE）
- 商品照片上传和展示(DONE)
- 商品上架和下架的功能实现(DONE)
- 商品实际金额的录入(DONE)
- 移除刷新按钮(DONE)
- 类别的展示基础文字，移除图标（DONE）
- 商场管理的星星价格展示，样式沿用任务管理的样式（DONE）
- 商城管理的类别展示，仅需显示文字，不需要样式（DONE）

#### 8 字典管理
- 功能：字典管理，可增删改查字典项。字段(字典类型，字典值，字典排序，字典状态，创建时间，备注)
- 字典类型增加‘商品类型’
- 字典管理的选择字典类型的card去掉，保留新增的按钮，样式跟商场管理应该是差不多的（DONE）
- 在新增字典项的弹出里，字典类型下拉框中，不需要显示“请选择字典类型”（DONE）
- 必填星号改为红的（DONE）
- 字典管理里隐藏系统自动生成的字典键，显示出来没有用，另外字典键的生产无需文本意义，自动生产随机码就行（DONE）
- 字典管理的字典类型改为可编辑的（DONE）
- 字典管理的弹窗风格与其他模块统一（TODO）
